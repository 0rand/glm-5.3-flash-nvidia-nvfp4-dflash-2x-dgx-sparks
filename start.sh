#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════════
# Boot GLM-5.3-Flash (NVIDIA official NVFP4) + DFlash2 k=5 on 2x DGX Spark, TP=2.
#
# Everything comes from .env — no site values in this script.
# The proven configuration lives there too; see docs/FINDINGS.md for WHY.
#
#   ./start.sh
#
# Boots in the background, waits for /health with an early abort if the container
# dies (a failed boot tears its own container down; waiting the full timeout
# against nothing is how you lose 40 minutes).
# ═══════════════════════════════════════════════════════════════════════════════
set -euo pipefail
